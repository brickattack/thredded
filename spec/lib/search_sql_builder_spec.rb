...
describe SearchSqlBuilder, 'build' do
  it 'finds results for a simple text search' do
    ...
    topic.posts.create!(topic: topic, messageboard: topic.messageboard,
                        content: 'alpine spring is very refreshing')
  ...
  it 'finds only 1 result with similar posts in different categories' do
    ...
    topic1 = create(:topic, messageboard: messageboard,
                    title: 'A funny category topic', with_categories: 1)
    topic2 = create(:topic, messageboard: messageboard,
                    title: 'A beer category topic', with_categories: 1)

    topic1.posts.create!(topic: topic1, messageboard: topic1.messageboard,
                         content: 'spring board cats')
  ...
  it 'finds only 1 result with similar posts by different users' do
    ...
    topic1 = create(:topic, messageboard: messageboard,
                    title: 'Joel made a topic', user: joel)
    topic2 = create(:topic, messageboard: messageboard,
                    title: 'Shaun made a topic', user: shaun)
  ...
  it 'finds results with more than one specified category' do
    messageboard = create(:messageboard)
    topic1 = create(:topic, messageboard: messageboard,
                    title: 'topic with more than 1 category',
                    with_categories: 2, with_posts: 2)
